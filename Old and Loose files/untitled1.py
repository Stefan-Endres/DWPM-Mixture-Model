...
def del_g_v_min_l0(s,p):
    ...
    g_R_v(s.m,p.m) - s.c[1]['x']*g_R_v(s.c[1],p.c[1]) \
    - s.c[2]['x']*g_R_v(s.c[2],p.c[2])
    
    g_R_l(s.m,p.m) - s.c[1]['x']*g_R_l(s.c[1],p.c[1]) \
    - s.c[2]['x']*g_R_l(s.c[2],p.c[2])
    
    del_g_IG_v(s.c[1],s.c[2])
    del_g_v_min_l0(s.c[1],p.c[1])
    """
...
def phase_split(s,p,x_r=100):
    ...
    s.c[1]['y'],s.c[2]['y'] = s.s['Data y1'],(1.0 -s.s['Data y1'])
...
if __name__ == '__main__':
    ...
    for p.m['k12'] in linspace(1e-30,0.1,30): # k12 = [-0.5, 0.5] #  (0.5,0.51
        #p.m['k12'] = p.m['k12']/1200.0
        print p.m['k12']
        s, p = phase_split(s,p,x_r=200)
    #%%
        #p.m['k12'] =  linspace(1e-30,0.1,30)[28]
        #s, p = phase_split(s,p,x_r=200)
        #p.m['k12'] =  0.11724137931034484
        #s, p = phase_split(s,p,x_r=200)

        #p.m['k12'] =  0.1034482758620689
        #s, p = phase_split(s,p,x_r=200)
      #%%  
